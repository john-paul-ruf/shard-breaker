...
| Types | `npm run typecheck` | `tsc -b --pretty false` | Post-S01: FAILS — `src/app/App.test.tsx` `createMemoryRepository` mock missing `saveCheckpoint` (S01 interface change, S02-CP2 owns the fix in `App.test.tsx`). Pre-existing debt to be cleared by S02. | red (pre-existing seam) |
| Unit/component | `npm run test:unit` | Discovers `src/**/*.test.ts(x)` | Post-S01: 217 passed across 12 files. | verified |
